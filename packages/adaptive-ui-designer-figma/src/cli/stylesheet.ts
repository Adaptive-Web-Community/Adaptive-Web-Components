...
export class StyleSheet implements IStylesheet {
  ...
  private constructor(private anatomy: IAnatomyConfiguration, private logger: ILogger) {
    this.path = path.resolve(path.dirname(anatomy.path), `../stylesheets/${spinalCase(anatomy.name)}.css`);
  }
  ...
  public async render(): Promise<void> {
    try {
      ...
      this.logger.success(`Writing stylesheet file for ${spinalCase(this.anatomy.name)}.`);
    } catch (e) {
